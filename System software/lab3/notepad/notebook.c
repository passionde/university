...
HWND CreateNewMDIChild(HWND hMDIClient)
{
	...
	hChild = (HWND)SendMessageW(hMDIClient, WM_MDICREATE, 0, (LPARAM)&mcs);
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	switch(msg)
	{
		case WM_CREATE:
		{
			...
			hSubMenu = CreatePopupMenu();
			AppendMenu(hSubMenu, MF_STRING, ID_EDIT_CUT, "C&ut");
			...
			hSubMenu = CreatePopupMenu();
			AppendMenu(hSubMenu, MF_STRING, ID_REFERENCE_INFO, "&Info");
			AppendMenu(hMenu, MF_STRING | MF_POPUP, (UINT)hSubMenu, "&Reference");

			SetMenu(hwnd, hMenu);

			// Set icons
			hIcon = LoadImage(NULL, "editor.ico", IMAGE_ICON, 32, 32, LR_LOADFROMFILE);
			SendMessage(hwnd, WM_SETICON, ICON_BIG, (LPARAM)hIcon);

			hIconSm = LoadImage(NULL, "editor.ico", IMAGE_ICON, 16, 16, LR_LOADFROMFILE);
			...
			SendMessage(hStatus, SB_SETTEXT, 0, (LPARAM)"Welcome");
		}
		break;
		case WM_SIZE:
		{
		...
		case WM_COMMAND:
			switch(LOWORD(wParam))
			{
				...
				break;
				case ID_REFERENCE_INFO: //ToDo
				    MessageBox(NULL, "A text code editor that we were forced to write in C.\nIt can do everything that a regular editor can: use hotkeys to work with text, open multiple files in one window, control their location", "About the program", MB_ICONASTERISK);
				break;
				default:
				{
...
LRESULT CALLBACK MDIChildWndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	switch(msg)
	{
		...
		//return DefMDIChildProc(hwnd, msg, wParam, lParam); TODO
		default:
